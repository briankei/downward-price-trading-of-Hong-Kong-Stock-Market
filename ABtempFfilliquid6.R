...
for(j in 1L:nrow(pd1)){
  ...
  if(length(BeginDayIndex)>0 && length(EndDayIndex)){
    ...
    pd1[j,"pilliquid"] = mean(dailyReturn, na.rm = TRUE)
  ...
  if(length(BeginDayIndex)>0 && length(EndDayIndex)){
    ...
    pd1[j,"filliquid"] = mean(dailyReturn, na.rm = TRUE)
  ...
  ###############################################################################
  #
  pdataF[,pd1Code]= pd1$filliquid
  pdataF2[,pd1Code]= pd1$pilliquid
...
pdataF$X <-NULL
write.csv(pdataF,file="AB1tempFfilliquid6.csv")
pdataF2$X <-NULL
write.csv(pdataF2,file="AB1tempFpilliquid6.csv")
...
pdataF$X <-NULL
write.csv(pdataF,file="AB2tempFfilliquid6.csv")
pdataF2$X <-NULL
write.csv(pdataF2,file="AB2tempFpilliquid6.csv")
