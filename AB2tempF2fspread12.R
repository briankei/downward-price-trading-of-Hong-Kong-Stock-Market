...
for(j in 1L:nrow(pd1)){
  ...
  if(length(BeginDayIndex)>0 && length(EndDayIndex)){
    ...
    pd1[j,"p2spread"] = mean(dailyReturn, na.rm = TRUE)
  ...
  if(length(BeginDayIndex)>0 && length(EndDayIndex)){
    ...
    pd1[j,"f2spread"] = mean(dailyReturn, na.rm = TRUE)
  ...
  ###############################################################################
  #
  pdataF[,pd1Code]= pd1$f2spread
  pdataF2[,pd1Code]= pd1$p2spread
...
beep("coin")
pdataF$X <-NULL
write.csv(pdataF,file="AB1tempF2fspread12.csv")
pdataF2$X <-NULL
write.csv(pdataF2,file="AB1tempF2pspread12.csv")
...
beep("coin")
pdataF$X <-NULL
write.csv(pdataF,file="AB2tempF2fspread12.csv")
pdataF2$X <-NULL
write.csv(pdataF2,file="AB2tempF2pspread12.csv")
