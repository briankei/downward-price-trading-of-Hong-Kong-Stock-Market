...
spread = read.csv("spread1.csv",header = T)
#spread2 = read.csv("spread2.csv",header = T)
ptemp <- temp
...
for(j in 1L:last){
  ...
  for(i in 1L: length(ExDate)){
    ...
    a <- spread[,codename]
    b <- as.Date(as.character(spread[,datename]),format="%m/%d/%Y")
...
temp$X<-NULL
write.csv(temp,"temp1postAvgSpread.csv")
ptemp$X<-NULL
write.csv(ptemp,"temp1preAvgSpread.csv")



################################ 2nd f #############################
temp= read.csv("temp2.csv",header = T)
spread = read.csv("spread2.csv",header = T)
#spread2 = read.csv("spread2.csv",header = T)
ptemp <- temp
...
temp$X<-NULL
write.csv(temp,"temp2postAvgSpread.csv")
ptemp$X<-NULL
write.csv(ptemp,"temp2preAvgSpread.csv")
