...
#  }
  ...
  if(length(BeginDayIndex)>0 && length(EndDayIndex)){
    ...
    pd1[j,"Return24months"] = exp(sum(dailyReturn, na.rm = TRUE))
  }
  ...
  ###############################################################################
  #
  pdataF[,pd1Code]= pd1$Return24months
...
beep("coin")
pdataF$X <-NULL
write.csv(pdataF,file="AB1temp24mR4.csv")
########-----------------------------------------2 file -----------------------------
...
beep("coin")
pdataF$X <-NULL
write.csv(pdataF,file="AB2temp24mR4.csv")
