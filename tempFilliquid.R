...
ptemp <- temp
illiquid = read.csv("illiq1.csv",header = T)
#illiquid2 = read.csv("illiq2.csv",header = T)
...
for(j in 1L:last){
  ...
  for(i in 1L: length(ExDate)){
    ...
    a <- illiquid[,codename]
    b <- as.Date(as.character(illiquid[,datename]),format="%m/%d/%Y")
    ...
    #Bloomberg data may not have the trading data on Ex-date
    c1 <- which(b > ExDate[i])
    if(length(c1)>0) {
      c2 <- which(b >= f366d)
      if(length(c2)>0) {
...
temp$X<-NULL
write.csv(temp,"temp1Filliquid.csv")
ptemp$X<-NULL
write.csv(ptemp,"temp1Fpilliquid.csv")
...
ptemp <- temp
illiquid = read.csv("illiq2.csv",header = T)
#illiquid2 = read.csv("illiq2.csv",header = T)
...
temp$X<-NULL
write.csv(temp,"temp2Filliquid.csv")
ptemp$X<-NULL
write.csv(ptemp,"temp2Fpilliquid.csv")
