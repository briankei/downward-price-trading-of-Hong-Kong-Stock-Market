...
monthreturn1 = read.csv("CUMULATIVE_TOT_RETURN_1Md.csv",header = T) 
...
#    }
    ...
    if(length(BeginDayIndex)>0 && length(EndDayIndex)){
      ...
      dailyReturn <- log(1+(dailyReturn/100)) #bloomberg return was in percentage. change it to 1+ratio
...
#daily record of adjusted monthyly return dated on monthend in % with NA on holidays
monthreturn1 = read.csv("CUMULATIVE_TOT_RETURN_2Md.csv",header = T) 
#monthreturn2 = read.csv("CUMULATIVE_TOT_RETURN_2Md.csv",header = T)
###############################################################################
#
CodeNames <- colnames(pd1)
